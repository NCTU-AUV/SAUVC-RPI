...
build_container:
	...
	docker exec $(CONTAINER_NAME) /bin/bash -i -c "cd $(WORKSPACE) && colcon build --symlink-install"
...
enter_container:
	...
	docker exec -it $(CONTAINER_NAME) /bin/bash -i -c "source $(WORKSPACE)/install/setup.bash && /bin/bash"

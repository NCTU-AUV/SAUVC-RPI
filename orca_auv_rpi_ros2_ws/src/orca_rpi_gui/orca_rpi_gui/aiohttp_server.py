...
class AIOHTTPServer:
    async def respond_index(request):
        return web.FileResponse(path=Path(orca_rpi_gui.__file__).parent/"index.html")

    async def respond_controller(request):
        return web.FileResponse(path=Path(orca_rpi_gui.__file__).parent/"controller.html")
    ...
    def __init__(self, msg_callback):
        ...
        app.router.add_static('/static/', Path(orca_rpi_gui.__file__).parent)

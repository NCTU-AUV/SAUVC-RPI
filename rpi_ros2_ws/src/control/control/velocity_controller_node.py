...
class VelocityPIDControllerNode(Node):
    ...
    Subscribes:
      - measured_topic (Float64MultiArray): default "camera/bottom/velocity_mps"
          expected format: [vx, vy, wz] (customize via measured_index)
      - target_topic (Float64): optional live target speed, default "control/targets/speed_mps"
        (If you don't publish target_topic, it will use parameter target_speed_mps.)

    Publishes:
      - output_topic (Wrench): default "control/wrench_sources/velocity"
    ...
    def __init__(self):
        ...
        self.declare_parameter("target_topic", "control/targets/speed_mps")  # optional
        self.declare_parameter("output_topic", "control/wrench_sources/velocity")
